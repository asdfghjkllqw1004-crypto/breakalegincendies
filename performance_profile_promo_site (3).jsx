...
const rehearsalPhotos = [
  ...
  },
  {
    ...
    image: "/images/rehearsal-scene.jpg",
  },
  {
    title: "리딩 연습",
    caption: "텍스트를 따라 인물의 시간을 읽어가는 순간",
    image: "/images/rehearsal-reading.jpg",
  },
  {
    title: "스텝 회의",
    caption: "무대 뒤에서 공연의 구조를 맞춰가는 순간",
    image: "/images/rehearsal-staff.jpg",
  },
];
...
function ProfileInitial({ name, image, fallbackImage, imagePosition = DEFAULT_IMAGE_POSITION }) {
  ...
  const shouldShowImage = currentImage && !imageFailed;
...
function RehearsalImageCard({ photo, index }) {
  const [imageFailed, setImageFailed] = useState(false);
  const isLarge = photo.size === "large";

  return (
    <div
      className={`group relative overflow-hidden border border-white/10 bg-white/[0.05] shadow-2xl shadow-black/30 ${
        isLarge ? "min-h-[330px] rounded-[2rem]" : "min-h-[135px] rounded-3xl"
      }`}
    >
      {!imageFailed ? (
        <img
      ...
      <div className={`absolute left-5 right-5 ${isLarge ? "bottom-6" : "bottom-5"}`}>
        <p className="mb-2 text-xs font-bold uppercase tracking-[0.22em] text-red-200">Rehearsal</p>
        <h3 className={`${isLarge ? "text-2xl" : "text-xl"} font-black leading-tight text-white`}>{photo.title}</h3>
        <p className="mt-2 text-xs leading-5 text-stone-300">{photo.caption}</p>
      </div>
    </div>
  );
}
      ...
      {selected && (
        ...
        >
          <motion.div
            ...
            className="relative grid w-full max-w-[430px] overflow-hidden rounded-3xl border border-white/10 bg-[#120d0b] shadow-2xl"
          >
            <button
              onClick={onClose}
            ...
            <div className={`relative min-h-[320px] bg-gradient-to-br ${selected.color || "from-stone-900 to-red-950"}`}>
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <ProfileInitial name={selected.name} image={selected.image} fallbackImage={selected.fallbackImage} imagePosition={selected.imagePosition} />
              </div>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 pb-14 pt-14">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }} className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2.2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/40 backdrop-blur">
              <div className="aspect-[3/4] overflow-hidden rounded-[1.7rem] bg-[radial-gradient(circle_at_40%_20%,rgba(254,202,202,0.24),transparent_28%),linear-gradient(135deg,#2b0b08,#090605_55%,#581c1c)] p-6">
      ...
      <section id="rehearsal" className="px-5 py-16">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-5">
            <RehearsalImageCard photo={rehearsalPhotos[0]} index={0} />

            <div className="grid gap-4">
              {rehearsalPhotos.slice(1).map((photo, index) => (
                <RehearsalImageCard key={photo.title} photo={photo} index={index} />
              ))}
            </div>
          </div>
        </div>
