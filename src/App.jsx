...
function ProfileInitial({ name, image, fallbackImage, imagePosition = DEFAULT_IMAGE_POSITION }) {
  ...
  return (
    <div className={`${profileIconClass} bg-black/20`} style={{ width: `${PROFILE_ICON_SIZE}px`, height: `${PROFILE_ICON_SIZE}px`, minWidth: `${PROFILE_ICON_SIZE}px`, minHeight: `${PROFILE_ICON_SIZE}px`, maxWidth: `${PROFILE_ICON_SIZE}px`, maxHeight: `${PROFILE_ICON_SIZE}px`, flexBasis: `${PROFILE_ICON_SIZE}px`, fontSize: PROFILE_ICON_FONT_SIZE, boxSizing: "border-box", lineHeight: 1 }}>
      {shouldShowImage ? (
        <img src={currentImage} alt={`${name} 프로필 사진`} className="h-full w-full object-cover" style={{ objectPosition: imagePosition }} loading="lazy" onError={() => { if (fallbackImage && currentImage !== fallbackImage) { setCurrentImage(fallbackImage); return; } setImageFailed(true); }} />
      ) : (
...
function RehearsalImageCard({ photo, index }) {
  ...
  return (
    <div className={`group relative overflow-hidden border border-white/10 bg-white/[0.05] shadow-2xl shadow-black/30 ${isLarge ? "min-h-[330px] rounded-[2rem]" : "min-h-[135px] rounded-3xl"}`}>
      {!imageFailed ? (
        <img src={photo.image} alt={photo.title} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" style={{ objectPosition: "center center" }} loading="lazy" onError={() => setImageFailed(true)} />
      ) : (
    ...
    <AnimatePresence>
      {selected && (
        <motion.div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-3 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
          <motion.div initial={{ y: 40, opacity: 0, scale: 0.98 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 30, opacity: 0, scale: 0.98 }} transition={{ type: "spring", stiffness: 260, damping: 24 }} onClick={(e) => e.stopPropagation()} className="relative grid w-full max-w-[430px] overflow-hidden rounded-3xl border border-white/10 bg-[#120d0b] shadow-2xl">
            ...
            <div className={`relative min-h-[320px] bg-gradient-to-br ${selected.color || "from-stone-900 to-red-950"}`}>
              ...
              <div className="absolute inset-0 flex items-center justify-center"><ProfileInitial name={selected.name} image={selected.image} fallbackImage={selected.fallbackImage} imagePosition={selected.imagePosition} /></div>
              <div className="absolute bottom-6 left-6 right-6"><p className="mb-2 text-sm font-medium text-red-200">{selected.role}</p><h2 className="text-4xl font-black tracking-tight text-white">{selected.name}</h2></div>
            </div>
...
export default function PerformanceProfilePromoSite() {
  ...
  const filteredActors = useMemo(() => actors.filter((person) => `${person.name} ${person.role} ${person.character}`.toLowerCase().includes(query.trim().toLowerCase())), [query]);
  const filteredStaff = useMemo(() => staff.filter((person) => `${person.name} ${person.role}`.toLowerCase().includes(query.trim().toLowerCase())), [query]);
    ...
    <main className="mx-auto min-h-screen w-full max-w-[430px] overflow-hidden bg-[#0b0807] text-stone-100 shadow-2xl shadow-black/40">
      <section className="relative isolate px-5 py-8">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 pb-14 pt-14">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }} className="relative"><div className="absolute -inset-5 rounded-[2.5rem] bg-red-700/20 blur-3xl" /><div className="relative overflow-hidden rounded-[2.2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/40 backdrop-blur"><div className="aspect-[3/4] overflow-hidden rounded-[1.7rem] bg-[radial-gradient(circle_at_40%_20%,rgba(254,202,202,0.24),transparent_28%),linear-gradient(135deg,#2b0b08,#090605_55%,#581c1c)] p-6"><div className="flex h-full flex-col justify-between"><div className="flex justify-between text-sm text-red-100/80"><span>Official Archive</span><span>2026</span></div><div className="space-y-4"><div className="h-1 w-24 rounded-full bg-red-200/70" /><h2 className="text-5xl font-black tracking-[-0.06em] text-white">화염</h2><p className="max-w-xs text-sm leading-6 text-stone-300">배우와 스텝의 이름이 하나의 불씨처럼 모여,<br />관객에게 공연의 첫인상을 남깁니다.</p></div><div className="grid grid-cols-2 gap-3">{["14 Actors", "6 Staff"].map((item) => <div key={item} className="rounded-2xl border border-white/10 bg-black/25 p-3 text-center text-xs font-semibold text-stone-200">{item}</div>)}</div></div></div></div></motion.div>
        </div>
      </section>
